...
const Signup = () => {
  ...
  const [validPassword,setValidPassword] = React.useState("hidden");
    const [passwordObj,setPasswordObj] = React.useState({isValid:false,err:""});
  const handleOAuth = () => {
...
function validatePassword(password, password2) {
  ...
  if(password===password2){
    ...
    if (!isValid) {
        if (password.length < 8) {
            ...
            err = 'Password should contain at least one lowercase letter, one uppercase letter, one digit, and one special character.';
        }
    }
    return {err,password};
  ...
  const handleClick = (e) => {
    ...
    setPasswordObj(validatePassword(password,password2));
        ...
        <div className="mx-auto w-full max-w-md bg-[#fff] px-6 pt-10 pb-8 shadow-2xl ring-1 ring-gray-900/5 sm:rounded-xl sm:px-10 rounded-2xl">
          <div className="w-full">
            ...
            <Link to={"/ts"}>
              ...
              <p className="mt-2 text-gray-500">Signup</p>
            </div>
            <div className="mt-5">
              <form action="" onSubmit={(e)=>{handleClick(e)}}>
                ...
                <div className="my-6 flex flex-col space-y-5">
                  ...
                  >
                    <div className={`${login} text-lg`}>Signup</div>
